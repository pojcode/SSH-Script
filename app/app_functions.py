    ...
    try:
        ...
        for i, c in enumerate(final_command_list):
                ...
                if (
                    ...
                    for v in files_to_check_list:
                        if v in output_dir and v in c:
                            ...
                            file_dir:str = ssh_connect.send_command(
                                f'dir | i {v}'
                            ).strip()
            ...
            if send_commmand:
                ...
                output_c:str = ssh_connect.send_command_timing(c,
                                                               read_timeout=0)
                if is_cisco:
                    if (
                        ...
                        buffer:str = ''
                        while True:
                            buffer += ssh_connect.read_channel()
                            if 'y/n' in buffer.lower():
                                index_yn:int = buffer.lower().index('y/n')
                                buffer = buffer[:index_yn + 5].strip()
                                ssh_connect.send_command_timing('y')
                                output_c = f'{buffer} y\n'
                                break
                            elif (
                                'nothing' in buffer.lower() and 
                                'clean' in buffer.lower()
                            ):
                                output_c = buffer
                                break
                        buffer = ssh_connect.read_until_prompt(read_timeout=0)
                        output_c += (
                            buffer.replace(ssh_connect.base_prompt, '')
                        )
                        ssh_connect.send_command('\n') # Avoid weird prompt
