...
def main():
    ...
    if HOST_OS == 'nt':  # Windows
        ...
        sep = '//'
    banner_title:str = (
        f'>>|    {THIS_FILE} RUNNING -- You can exit script pressing '
        '"Ctrl+C" or closing window at any time    |<<'
    )
    ...
    try:
       ...
       DATABASE:dict = create_db(data_file_raw)
    ...
    for c in command_list:
        if '<file=' in c:
            config_f_info:dict|list[str] = search_config_files(c, 
                                                               config_f_path)
    ...
    try:
        cfg_config:list[str] = load_file(CFG_FILE, MAIN_PATH)
    except FileNotFoundError:
        ...
        cfg_db_temp:dict = create_cfg_db(cfg_config)
    ...
    for i in range(2):  # i=0 -> HOST, i=1 -> MISC
        ...
        if i == 1:
            ...
            keyword:str = 'HOST'
        ip_data:list[list[str]] = DATABASE.get(f'{keyword}DATA')
        for v in ip_data:
            if len(v) == 1:
                ip_list.append(v[0])
            else:
    ...
    if do_ping.lower() in ('y', 'ye', 'yes'):
        for i in range(2):  # i=0 -> HOST, i=1 -> MISC
            ...
            log_global_summary += (
                ...
                f'{" "*6}- SUCCESSFUL {keyword} PINGS -> {counter}\n'
            )
        ...
        for ip in ip_ssh_list:
            ...
            futures.append(
                exec.submit(
                    ...
                    device, 
                    DATABASE, 
                    cfg_db.get('reload_and_go_timeout')
                )
            )
    ...
    log_global_summary += f'\t- TOTAL SSH SESSIONS -> {len(ip_ssh_list)}' \
                          f'{" "*6}- ESTABLISHED SSH SESSIONS -> {counter}'
